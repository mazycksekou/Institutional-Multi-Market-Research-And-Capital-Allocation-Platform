...
def run_feature_ablation_lab(*args: Any, **kwargs: Any) -> dict[str, Any]:
    ...
    result = legacy_run_feature_ablation_lab(*args, **kwargs)
    ...
    active_fields = set(result.get("active_fields") or [])
    active_fields.update(_all_safe_fields_for_combination(sport, market))
    result["active_fields"] = sorted(active_fields)
    return result
...
def run_calibration_strategy_filter(*args: Any, **kwargs: Any) -> dict[str, Any]:
    ...
    return legacy_run_calibration_strategy_filter(*args, **kwargs)
