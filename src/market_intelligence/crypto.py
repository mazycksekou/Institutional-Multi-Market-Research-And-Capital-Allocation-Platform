...
def build_crypto_intelligence_report(payload: Mapping[str, Any] | None = None, /, **overrides: Any) -> dict[str, Any]:
    ...
    return build_market_intelligence_report(
        {
            "market": "crypto",
            "symbol_or_event": data.get("symbol") or data.get("asset") or "",
            ...
            "trade_plan": data.get("trade_plan") or "Review only; no live crypto execution.",
            "risk": data.get("risk") or "low",
            "invalidation": data.get("invalidation") or "trend regime breaks or liquidity collapses",
        }
    )
