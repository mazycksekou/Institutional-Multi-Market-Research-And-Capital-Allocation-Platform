...
def evaluate_settlement_liquidity_gate(**kwargs):
    ...
    settlement_rule_status = "approved" if kwargs.get("prediction_market_resolution_match", True) else "blocked_by_governance"
    return {
        **kwargs,
        "execution_feasibility_score": float(kwargs.get("execution_feasibility_score", kwargs.get("fill_probability", 0) * 100)),
        "settlement_rule_status": settlement_rule_status,
        "gate_result": "approved" if ok else "blocked_by_governance",
    }
