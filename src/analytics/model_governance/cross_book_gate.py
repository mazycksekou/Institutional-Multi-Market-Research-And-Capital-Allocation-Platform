...
def evaluate_cross_book_gate(**kwargs):
    blocked = []
    if float(kwargs.get("market_identity_confidence", 100)) < 85: blocked.append("low_market_identity")
    ...
    if kwargs.get("single_book_only", False): blocked.append("single_book_only")
    if kwargs.get("same_side_only", False): blocked.append("same_side_only")
