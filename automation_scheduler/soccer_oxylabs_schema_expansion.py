...
from .soccer_schema_expansion import build_soccer_schema_expansion_report


REPORT_ROOT = Path("reports")


def build_soccer_oxylabs_schema_expansion_report(
    *,
    ...
    base = build_soccer_schema_expansion_report(sample_verification_results=sample_verification_results)
    ...
    lane_index = {lane["lane_name"]: lane for lane in soccer_lane_catalog()}
    rows = []
    for entry in base.get("new_fields_created") or []:
        lane_name = next((lane["lane_name"] for lane in soccer_lane_catalog() if lane["table"] == entry["table"]), "")
        ...
        rows.append(
            {
                **entry,
                ...
                "tests": ["tests/test_soccer_schema_expansion.py", "tests/test_soccer_oxylabs_schema_expansion.py"],
            }
        )
    return {
        "ok": True,
        "status": "ok",
        "report_name": "SOCCER_OXYLABS_SCHEMA_EXPANSION_REPORT",
        "schema_version": "soccer_oxylabs_schema_expansion_v1",
...
def write_soccer_oxylabs_schema_expansion_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "SOCCER_OXYLABS_SCHEMA_EXPANSION_REPORT.json"
    md_path = root / "SOCCER_OXYLABS_SCHEMA_EXPANSION_REPORT.md"
    write_json(json_path, report)
    lines = [
        "# Soccer Oxylabs Schema Expansion Report",
        "",
