...
def build_wnba_backfill_loader_report() -> dict[str, Any]:
    return build_basketball_loader_ready_backfill_report(sport=SPORT)


def build_wnba_backfill_loader() -> dict[str, Any]:
    return build_wnba_backfill_loader_report()


def write_wnba_backfill_loader_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    return write_basketball_loader_ready_backfill_report(report, output_dir=output_dir)


def build_and_write_wnba_backfill_loader_report(*, output_dir: str | Path | None = None) -> dict[str, Any]:
