...
from .soccer_free_data_loader import default_soccer_loader_lanes, load_soccer_lane_records
from .soccer_oxylabs_common import SOCCER_DATA_ROOT, current_utc, lane_final_state, stable_hash, write_json, write_md
...
def build_soccer_loader_ready_backfill_report() -> dict[str, Any]:
    lanes = default_soccer_loader_lanes()
    session_stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    session_id = sanitize_filename(f"soccer_loader_{session_stamp}_{stable_hash({'sport': 'soccer'})[:8]}")
    session_root = SOCCER_DATA_ROOT / "backfill_sessions" / session_id
    ...
    report = {
        "ok": True,
        "status": "ok",
        "report_name": "SOCCER_LOADER_READY_BACKFILL_REPORT",
        "schema_version": "soccer_loader_ready_backfill_v1",
...
def write_soccer_loader_ready_backfill_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "SOCCER_LOADER_READY_BACKFILL_REPORT.json"
    md_path = root / "SOCCER_LOADER_READY_BACKFILL_REPORT.md"
    write_json(json_path, report)
    lines = [
        "# Soccer Loader Ready Backfill Report",
        "",
        ...
        f"4. records_added_by_soccer: {report.get('records_added_by_soccer')}",
