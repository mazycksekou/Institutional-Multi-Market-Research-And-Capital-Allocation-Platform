...
from .tennis_oxylabs_common import REPORT_ROOT, current_utc, fetch_public_page_text, stable_hash, url_hash, write_json, write_md
from .tennis_source_policy_review import tennis_candidate_source_catalog
...
def sample_tennis_source(candidate: dict[str, Any], matrix_row: dict[str, Any]) -> dict[str, Any]:
...
def build_tennis_safe_source_sample_report(
    *,
    ...
    rows = [
        sample_tennis_source(index[row["source_id"]], row)
    ...
    return {
        "ok": True,
        "status": "ok",
        "report_name": "TENNIS_SAFE_SOURCE_SAMPLE_REPORT",
        "schema_version": "tennis_safe_source_sample_report_v1",
...
def write_tennis_safe_source_sample_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "TENNIS_SAFE_SOURCE_SAMPLE_REPORT.json"
    md_path = root / "TENNIS_SAFE_SOURCE_SAMPLE_REPORT.md"
    write_json(json_path, report)
    lines = [
        "# Tennis Safe Source Sample Report",
        "",
