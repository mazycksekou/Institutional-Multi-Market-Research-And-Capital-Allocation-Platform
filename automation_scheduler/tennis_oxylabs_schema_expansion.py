...
from .tennis_schema_expansion import build_tennis_schema_expansion_report


REPORT_ROOT = Path("reports")


def build_tennis_oxylabs_schema_expansion_report(
    *,
    sample_verification_results: dict[str, Any] | None = None,
    schema_report: dict[str, Any] | None = None,
) -> dict[str, Any]:
    sample_verification_results = sample_verification_results or build_tennis_targeted_sample_verification_results()
    ...
    for entry in schema_report.get("new_fields_created") or []:
        lane_name = ""
        for key, sample in sample_index.items():
            if sample.get("validation_status") == "sample_verified" and entry.get("table") in str(key):
    ...
    return {
        "ok": True,
        "status": "ok",
        "report_name": "TENNIS_OXYLABS_SCHEMA_EXPANSION_REPORT",
        "schema_version": "tennis_oxylabs_schema_expansion_v1",
...
def write_tennis_oxylabs_schema_expansion_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "TENNIS_OXYLABS_SCHEMA_EXPANSION_REPORT.json"
    md_path = root / "TENNIS_OXYLABS_SCHEMA_EXPANSION_REPORT.md"
    write_json(json_path, report)
    lines = [
        "# Tennis Oxylabs Schema Expansion Report",
        "",
