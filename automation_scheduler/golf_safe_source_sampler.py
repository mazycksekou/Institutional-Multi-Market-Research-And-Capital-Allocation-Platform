...
from .golf_oxylabs_common import REPORT_ROOT, current_utc, fetch_public_page_text, stable_hash, url_hash, write_json, write_md
from .golf_source_policy_review import golf_candidate_source_catalog


ACCEPTED_SAMPLE_DECISIONS = {
    "accepted_for_automated_normalized_backfill",
    "accepted_for_postevent_training_only",
...
def sample_golf_source(candidate: dict[str, Any], matrix_row: dict[str, Any]) -> dict[str, Any]:
...
def build_golf_safe_source_sample_report(
    *,
    policy_matrix: dict[str, Any],
    candidate_rows: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
    candidate_rows = candidate_rows or golf_candidate_source_catalog()
    index = {candidate["source_id"]: candidate for candidate in candidate_rows}
    rows = [
        sample_golf_source(index[row["source_id"]], row)
    ...
    return {
        "ok": True,
        "status": "ok",
        "report_name": "golf_SAFE_SOURCE_SAMPLE_REPORT",
        "schema_version": "golf_safe_source_sample_report_v1",
...
def write_golf_safe_source_sample_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "GOLF_SAFE_SOURCE_SAMPLE_REPORT.json"
    md_path = root / "GOLF_SAFE_SOURCE_SAMPLE_REPORT.md"
    write_json(json_path, report)
    lines = [
        "# Golf Safe Source Sample Report",
        "",
