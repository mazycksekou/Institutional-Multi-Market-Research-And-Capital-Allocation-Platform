...
from .ncaaf_free_data_loader import load_ncaaf_lane_records
from .ncaaf_free_vs_paid_readiness import RUN_MODE, _new_fields_for_lane, ncaaf_lane_catalog
from .ncaaf_oxylabs_common import current_utc, lane_final_state, write_json, write_md
...
def _sample_lane(
    ...
    *,
    ...
    if final_state == "free_open_metadata_only" and lane["lane_name"] in {"team_metadata_entities", "postseason_metadata"}:
        ...
        return {
            ...
            "sample_type": "one_public_structured_entity",
    ...
    result = load_ncaaf_lane_records(lane, policy_matrix=policy_matrix, cache=cache)
    ...
    return {
        "sport": lane["sport"],
        "lane_name": lane["lane_name"],
        "sample_type": "one_game_drive_play_or_entity",
...
def build_ncaaf_targeted_sample_verification_results(
    *,
    ...
    cache: dict[str, Any] = {}
    rows = [_sample_lane(lane, policy_matrix=policy_matrix, sample_report=sample_report, cache=cache) for lane in ncaaf_lane_catalog()]
    return {
        "ok": True,
        "status": "ok",
        "report_name": "NCAAF_TARGETED_SAMPLE_VERIFICATION_RESULTS",
        "schema_version": "ncaaf_targeted_sample_verification_v1",
...
def write_ncaaf_targeted_sample_verification_results(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "NCAAF_TARGETED_SAMPLE_VERIFICATION_RESULTS.json"
    md_path = root / "NCAAF_TARGETED_SAMPLE_VERIFICATION_RESULTS.md"
    write_json(json_path, report)
    lines = [
        "# NCAAF Targeted Sample Verification Results",
        "",
