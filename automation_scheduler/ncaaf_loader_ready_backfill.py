...
from .ncaaf_free_vs_paid_readiness import default_ncaaf_loader_lanes
from .ncaaf_oxylabs_common import current_utc, data_session_root, lane_final_state, write_json, write_md
...
def build_ncaaf_loader_ready_backfill_report(*, policy_matrix: dict[str, Any] | None = None) -> dict[str, Any]:
    policy_matrix = policy_matrix or build_ncaaf_source_policy_matrix()
    lanes = default_ncaaf_loader_lanes()
    session_id, session_root = data_session_root("NCAAF_loader")
    cache: dict[str, Any] = {}
    ...
    report = {
        "ok": True,
        "status": "ok",
        "report_name": "NCAAF_LOADER_READY_BACKFILL_REPORT",
        "schema_version": "NCAAF_loader_ready_backfill_v1",
        ...
        "records_added_by_ncaaf": total_records,
...
def write_ncaaf_loader_ready_backfill_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "NCAAF_LOADER_READY_BACKFILL_REPORT.json"
    md_path = root / "NCAAF_LOADER_READY_BACKFILL_REPORT.md"
    write_json(json_path, report)
    lines = [
        "# NCAAF Loader Ready Backfill Report",
        "",
        ...
        f"4. records_added_by_ncaaf: {report.get('records_added_by_ncaaf')}",
