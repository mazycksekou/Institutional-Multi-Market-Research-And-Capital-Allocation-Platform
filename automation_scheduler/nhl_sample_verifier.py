...
from .nhl_free_vs_paid_readiness import RUN_MODE, _new_fields_for_lane, nhl_lane_catalog
from .nhl_oxylabs_common import current_utc, lane_final_state, write_json, write_md
...
def _sample_lane(lane: dict[str, Any], *, cache: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        "sport": lane["sport"],
        "lane_name": lane["lane_name"],
        "sample_type": "one_game"
        if lane.get("entity_level", "game") in {"game", "team_game", "player_game", "goalie_game", "play", "shot", "penalty_event", "official_game"}
        else "one_team",
        "sample_scope": "official_public_current_sample_scope",
...
def build_nhl_targeted_sample_verification_results() -> dict[str, Any]:
    cache: dict[str, Any] = {}
    rows = [_sample_lane(lane, cache=cache) for lane in nhl_lane_catalog()]
    return {
        "ok": True,
        "status": "ok",
        "report_name": "NHL_TARGETED_SAMPLE_VERIFICATION_RESULTS",
        "schema_version": "nhl_targeted_sample_verification_v1",
...
def write_nhl_targeted_sample_verification_results(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "NHL_TARGETED_SAMPLE_VERIFICATION_RESULTS.json"
    md_path = root / "NHL_TARGETED_SAMPLE_VERIFICATION_RESULTS.md"
    write_json(json_path, report)
    lines = [
        "# NHL Targeted Sample Verification Results",
        "",
