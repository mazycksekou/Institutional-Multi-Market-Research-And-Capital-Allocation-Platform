...
from .golf_oxylabs_common import FINAL_ACTIONABLE_STATES, current_utc, write_json, write_md
from .golf_safe_source_sampler import build_golf_safe_source_sample_report
from .golf_source_exhaustion_query_builder import build_golf_source_exhaustion_query_plan
from .golf_source_policy_review import build_golf_source_policy_matrix, golf_candidate_source_catalog
...
def _query_index() -> dict[str, list[dict[str, Any]]]:
    return build_golf_source_exhaustion_query_plan()["lane_query_index"]
...
def _lane_final_state(lane: dict[str, Any], policy_row: dict[str, Any], sample_row: dict[str, Any]) -> tuple[str, int]:
    ...
    if source_final_state == "free_open_metadata_only":
        return "free_open_metadata_only", 1 if sample_row.get("normalized_records_added") else 0
    if source_final_state == "free_open_backfilled":
        return "free_open_backfilled", 1
    return source_final_state or "unavailable_after_exhaustive_free_search", 0


def build_golf_oxylabs_source_exhaustion_log(
    *,
    ...
    sample_report = sample_report or build_golf_safe_source_sample_report(
        policy_matrix=policy_matrix,
        candidate_rows=golf_candidate_source_catalog(),
    )
    ...
    for lane in golf_lane_catalog():
        ...
        query_used = (query_index.get(lane_key) or [{}])[0].get("query") or f"golf {lane['lane_name']}"
        ...
        rows.append(
            {
                ...
                "normalized_records_found": 1 if records_added else 0,
    ...
    return {
        "ok": True,
        "status": "ok",
        "report_name": "golf_OXYLABS_SOURCE_EXHAUSTION_LOG",
        "schema_version": "golf_oxylabs_source_exhaustion_log_v1",
...
def write_golf_oxylabs_source_exhaustion_log(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "GOLF_OXYLABS_SOURCE_EXHAUSTION_LOG.json"
    md_path = root / "GOLF_OXYLABS_SOURCE_EXHAUSTION_LOG.md"
    write_json(json_path, report)
    lines = [
        "# Golf Oxylabs Source Exhaustion Log",
        "",
...
def build_golf_oxylabs_reclassification_report(*, source_exhaustion_report: dict[str, Any] | None = None) -> dict[str, Any]:
    report = source_exhaustion_report or build_golf_oxylabs_source_exhaustion_log()
    rows = []
    for row in report.get("source_candidate_rows") or []:
    ...
    return {
        "ok": True,
        "status": "ok",
        "report_name": "golf_OXYLABS_RECLASSIFICATION_REPORT",
        "schema_version": "golf_oxylabs_reclassification_report_v1",
        "created_at": current_utc(),
...
def write_golf_oxylabs_reclassification_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "GOLF_OXYLABS_RECLASSIFICATION_REPORT.json"
    md_path = root / "GOLF_OXYLABS_RECLASSIFICATION_REPORT.md"
    write_json(json_path, report)
    lines = [
        "# Golf Oxylabs Reclassification Report",
        "",
