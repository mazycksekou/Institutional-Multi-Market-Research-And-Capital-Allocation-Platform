...
from .combat_free_vs_paid_readiness import _new_fields_for_lane, combat_lane_catalog
from .combat_oxylabs_common import current_utc, write_json, write_md
from .combat_sample_verifier import build_combat_targeted_sample_verification_results
...
def _sample_index(sample_verification_results: dict[str, Any] | None) -> dict[str, dict[str, Any]]:
    if not sample_verification_results:
        sample_verification_results = build_combat_targeted_sample_verification_results()
...
def _schema_entry(lane: dict[str, Any], field_name: str, sample: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "description": f"{field_name} derived from {lane['field_or_feature_group']} for Combat",
        "entity_level": lane["entity_level"],
        "table": lane["table"],
        "data_type": "boolean" if field_name.endswith(("_flag", "_confirmed")) else "float" if field_name.endswith(("_rate", "_proxy", "_pct", "_split")) else "string",
        ...
        "tests": ["tests/test_combat_schema_expansion.py"],
        "report_entry": True,
    }


def build_combat_schema_expansion_report(*, sample_verification_results: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        "ok": True,
        "status": "ok",
        "report_name": "COMBAT_SCHEMA_EXPANSION_REPORT",
        "schema_version": "combat_schema_expansion_v1",
...
def write_combat_schema_expansion_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "COMBAT_SCHEMA_EXPANSION_REPORT.json"
    md_path = root / "COMBAT_SCHEMA_EXPANSION_REPORT.md"
    write_json(json_path, report)
    lines = [
        "# Combat Schema Expansion Report",
        "",
