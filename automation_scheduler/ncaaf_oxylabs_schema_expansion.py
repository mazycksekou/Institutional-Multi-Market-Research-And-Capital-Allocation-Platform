...
from .ncaaf_schema_expansion import build_ncaaf_schema_expansion_report


REPORT_ROOT = Path("reports")


def build_ncaaf_oxylabs_schema_expansion_report(
    *,
    sample_verification_results: dict[str, Any] | None = None,
    schema_report: dict[str, Any] | None = None,
) -> dict[str, Any]:
    sample_verification_results = sample_verification_results or build_ncaaf_targeted_sample_verification_results()
    schema_report = schema_report or build_ncaaf_schema_expansion_report(sample_verification_results=sample_verification_results)
    ...
    return {
        "ok": True,
        "status": "ok",
        "report_name": "NCAAF_OXYLABS_SCHEMA_EXPANSION_REPORT",
        "schema_version": "NCAAF_oxylabs_schema_expansion_v1",
...
def write_ncaaf_oxylabs_schema_expansion_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "NCAAF_OXYLABS_SCHEMA_EXPANSION_REPORT.json"
    md_path = root / "NCAAF_OXYLABS_SCHEMA_EXPANSION_REPORT.md"
    write_json(json_path, report)
    lines = [
        "# NCAAF Oxylabs Schema Expansion Report",
        "",
