...
from .combat_schema_expansion import build_combat_schema_expansion_report


REPORT_ROOT = Path("reports")


def build_combat_oxylabs_schema_expansion_report(
    *,
    sample_verification_results: dict[str, Any] | None = None,
    schema_report: dict[str, Any] | None = None,
) -> dict[str, Any]:
    sample_verification_results = sample_verification_results or build_combat_targeted_sample_verification_results()
    ...
    return {
        "ok": True,
        "status": "ok",
        "report_name": "COMBAT_OXYLABS_SCHEMA_EXPANSION_REPORT",
        "schema_version": "combat_oxylabs_schema_expansion_v1",
...
def write_combat_oxylabs_schema_expansion_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "COMBAT_OXYLABS_SCHEMA_EXPANSION_REPORT.json"
    md_path = root / "COMBAT_OXYLABS_SCHEMA_EXPANSION_REPORT.md"
    write_json(json_path, report)
    lines = [
        "# Combat Oxylabs Schema Expansion Report",
        "",
