...
from .tennis_free_vs_paid_readiness import RUN_MODE, _new_fields_for_lane, tennis_lane_catalog
from .tennis_oxylabs_common import current_utc, lane_final_state, write_json, write_md
...
def _sample_lane(
    ...
    *,
    ...
    if final_state == "free_open_metadata_only" and lane["lane_name"] == "player_metadata_handedness_country":
...
def build_tennis_targeted_sample_verification_results(
    *,
    ...
    cache: dict[str, Any] = {}
    rows = [_sample_lane(lane, policy_matrix=policy_matrix, sample_report=sample_report, cache=cache) for lane in tennis_lane_catalog()]
    return {
        "ok": True,
        "status": "ok",
        "report_name": "TENNIS_TARGETED_SAMPLE_VERIFICATION_RESULTS",
        "schema_version": "tennis_targeted_sample_verification_v1",
...
def write_tennis_targeted_sample_verification_results(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "TENNIS_TARGETED_SAMPLE_VERIFICATION_RESULTS.json"
    md_path = root / "TENNIS_TARGETED_SAMPLE_VERIFICATION_RESULTS.md"
    write_json(json_path, report)
    lines = [
        "# Tennis Targeted Sample Verification Results",
        "",
