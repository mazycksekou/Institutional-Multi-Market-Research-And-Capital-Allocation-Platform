...
from .nhl_free_data_loader import default_nhl_loader_lanes, load_nhl_lane_records
from .nhl_oxylabs_common import NHL_DATA_ROOT, current_utc, lane_final_state, stable_hash, write_json, write_md
...
def _hard_block_reason(result: dict[str, Any]) -> str:
    blocked_reason = str(result.get("blocked_reason") or "")
    if blocked_reason == "schema_mapping_failed":
        return "schema_mapping_failed"
    if blocked_reason == "validation_failed":
        return "validation_failed"
    if blocked_reason == "no_records_available":
        return "no_records_available"
    return "hard_blocked_source"


def build_nhl_loader_ready_backfill_report() -> dict[str, Any]:
    lanes = default_nhl_loader_lanes()
    session_stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    session_id = sanitize_filename(f"nhl_loader_{session_stamp}_{stable_hash({'sport': 'icehockey_nhl'})[:8]}")
    session_root = NHL_DATA_ROOT / "backfill_sessions" / session_id
        ...
        rows.append(
            {
                ...
                "hard_block_reason": None if backfill_written else _hard_block_reason(result),
    ...
    report = {
        "ok": True,
        "status": "ok",
        "report_name": "NHL_LOADER_READY_BACKFILL_REPORT",
        "schema_version": "nhl_loader_ready_backfill_v1",
        ...
        "records_added_by_nhl": total_records,
        "backfill_records_written_total": total_records,
        "fields_closed_this_pass": total_records,
        "fields_partially_closed_this_pass": sum(1 for row in rows if row["lane_name"] in {"power_play_penalty_kill_stats", "goalie_workload_rest", "rest_travel_features", "player_prop_feature_candidates"}),
...
def write_nhl_loader_ready_backfill_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "NHL_LOADER_READY_BACKFILL_REPORT.json"
    md_path = root / "NHL_LOADER_READY_BACKFILL_REPORT.md"
    write_json(json_path, report)
    lines = [
        "# NHL Loader Ready Backfill Report",
        "",
        ...
        f"4. records_added_by_nhl: {report.get('records_added_by_nhl')}",
