...
from .combat_oxylabs_common import REPORT_ROOT, current_utc, fetch_public_page_text, stable_hash, url_hash, write_json, write_md
from .combat_source_policy_review import combat_candidate_source_catalog
...
def sample_combat_source(candidate: dict[str, Any], matrix_row: dict[str, Any]) -> dict[str, Any]:
...
def build_combat_safe_source_sample_report(
    *,
    ...
    rows = [
        sample_combat_source(index[row["source_id"]], row)
    ...
    return {
        "ok": True,
        "status": "ok",
        "report_name": "COMBAT_SAFE_SOURCE_SAMPLE_REPORT",
        "schema_version": "combat_safe_source_sample_report_v1",
...
def write_combat_safe_source_sample_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "COMBAT_SAFE_SOURCE_SAMPLE_REPORT.json"
    md_path = root / "COMBAT_SAFE_SOURCE_SAMPLE_REPORT.md"
    write_json(json_path, report)
    lines = [
        "# Combat Safe Source Sample Report",
        "",
