...
from .soccer_oxylabs_source_policy import evaluate_soccer_oxylabs_source_policy
...
def _query_index() -> dict[str, list[dict[str, Any]]]:
    return build_soccer_source_exhaustion_query_plan()["lane_query_index"]


def _web_confirmation_for_lane(lane: dict[str, Any]) -> dict[str, Any]:
    source_id = str(lane.get("source_id") or "")
    if source_id == "soccer_football_data_csv":
        spec = lane_source_spec({"source_id": "soccer_football_data_docs"})
    elif source_id == "soccer_statsbomb_open_data":
        spec = lane_source_spec({"source_id": "soccer_statsbomb_open_repo"})
    else:
        spec = lane_source_spec(lane)
...
def _lane_candidate_row(lane: dict[str, Any], query_index: dict[str, list[dict[str, Any]]], *, cache: dict[str, Any]) -> dict[str, Any]:
    lane_key = f"{lane['sport']}::{lane['lane_name']}"
    query_used = (query_index.get(lane_key) or [{}])[0].get("query") or f"Soccer {lane['lane_name']}"
    source_spec = lane_source_spec(lane)
    policy = evaluate_soccer_oxylabs_source_policy(
    ...
    if lane["free_or_paid_category"] in {"free_open_populated", "free_open_partial"}:
        ...
        page_confirmation = _web_confirmation_for_lane(lane)
        web_ok = bool(page_confirmation.get("ok"))
        final_state = lane_final_state(lane, backfill_written=accepted, hard_blocked=not accepted)
        if web_ok:
            transport_used = "both"
        else:
            transport_used = result.get("oxylabs_transport_used") or "residential_proxy"
        return {
            "sport": lane["sport"],
            "lane_name": lane["lane_name"],
            "query_used": query_used,
    ...
    page_fetch = _web_confirmation_for_lane(lane)
...
def build_soccer_oxylabs_source_exhaustion_log() -> dict[str, Any]:
    query_index = _query_index()
    cache: dict[str, Any] = {}
    rows = [_lane_candidate_row(lane, query_index, cache=cache) for lane in soccer_lane_catalog()]
    return {
        "ok": True,
        "status": "ok",
        "report_name": "SOCCER_OXYLABS_SOURCE_EXHAUSTION_LOG",
        "schema_version": "soccer_oxylabs_source_exhaustion_log_v1",
...
def write_soccer_oxylabs_source_exhaustion_log(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "SOCCER_OXYLABS_SOURCE_EXHAUSTION_LOG.json"
    md_path = root / "SOCCER_OXYLABS_SOURCE_EXHAUSTION_LOG.md"
    write_json(json_path, report)
    lines = [
        "# Soccer Oxylabs Source Exhaustion Log",
        "",
...
def build_soccer_oxylabs_reclassification_report(*, source_exhaustion_report: dict[str, Any] | None = None) -> dict[str, Any]:
    report = source_exhaustion_report or build_soccer_oxylabs_source_exhaustion_log()
    rows = []
    for row in report.get("source_candidate_rows") or []:
    ...
    return {
        "ok": True,
        "status": "ok",
        "report_name": "SOCCER_OXYLABS_RECLASSIFICATION_REPORT",
        "schema_version": "soccer_oxylabs_reclassification_report_v1",
        "created_at": current_utc(),
...
def write_soccer_oxylabs_reclassification_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "SOCCER_OXYLABS_RECLASSIFICATION_REPORT.json"
    md_path = root / "SOCCER_OXYLABS_RECLASSIFICATION_REPORT.md"
    write_json(json_path, report)
    lines = [
        "# Soccer Oxylabs Reclassification Report",
        "",
