...
from .ncaaf_oxylabs_common import FINAL_ACTIONABLE_STATES, current_utc, write_json, write_md
from .ncaaf_safe_source_sampler import build_ncaaf_safe_source_sample_report
from .ncaaf_source_exhaustion_query_builder import build_ncaaf_source_exhaustion_query_plan
from .ncaaf_source_policy_review import build_ncaaf_source_policy_matrix, ncaaf_candidate_source_catalog
...
def _query_index() -> dict[str, list[dict[str, Any]]]:
    return build_ncaaf_source_exhaustion_query_plan()["lane_query_index"]
...
def build_ncaaf_oxylabs_source_exhaustion_log(
    *,
    ...
    sample_report = sample_report or build_ncaaf_safe_source_sample_report(
        policy_matrix=policy_matrix,
        candidate_rows=ncaaf_candidate_source_catalog(),
    )
    ...
    for lane in ncaaf_lane_catalog():
        ...
        query_used = (query_index.get(lane_key) or [{}])[0].get("query") or f"NCAAF {lane['lane_name']}"
    ...
    return {
        "ok": True,
        "status": "ok",
        "report_name": "NCAAF_OXYLABS_SOURCE_EXHAUSTION_LOG",
        "schema_version": "NCAAF_oxylabs_source_exhaustion_log_v1",
...
def write_ncaaf_oxylabs_source_exhaustion_log(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "NCAAF_OXYLABS_SOURCE_EXHAUSTION_LOG.json"
    md_path = root / "NCAAF_OXYLABS_SOURCE_EXHAUSTION_LOG.md"
...
def build_ncaaf_oxylabs_reclassification_report(*, source_exhaustion_report: dict[str, Any] | None = None) -> dict[str, Any]:
    report = source_exhaustion_report or build_ncaaf_oxylabs_source_exhaustion_log()
    rows = []
    for row in report.get("source_candidate_rows") or []:
    ...
    return {
        "ok": True,
        "status": "ok",
        "report_name": "NCAAF_OXYLABS_RECLASSIFICATION_REPORT",
        "schema_version": "NCAAF_oxylabs_reclassification_report_v1",
        "created_at": current_utc(),
...
def write_ncaaf_oxylabs_reclassification_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "NCAAF_OXYLABS_RECLASSIFICATION_REPORT.json"
    md_path = root / "NCAAF_OXYLABS_RECLASSIFICATION_REPORT.md"
    write_json(json_path, report)
    lines = [
