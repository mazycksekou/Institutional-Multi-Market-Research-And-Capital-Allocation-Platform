...
from .tennis_free_data_loader import load_tennis_lane_records
from .tennis_free_vs_paid_readiness import default_tennis_loader_lanes
from .tennis_oxylabs_common import current_utc, data_session_root, lane_final_state, write_json, write_md
...
def build_tennis_loader_ready_backfill_report(*, policy_matrix: dict[str, Any] | None = None) -> dict[str, Any]:
    policy_matrix = policy_matrix or build_tennis_source_policy_matrix()
    lanes = default_tennis_loader_lanes()
    session_id, session_root = data_session_root("tennis_loader")
    cache: dict[str, Any] = {}
    rows: list[dict[str, Any]] = []
    total_records = 0
    for lane in lanes:
        result = load_tennis_lane_records(lane, policy_matrix=policy_matrix, cache=cache)
    ...
    report = {
        "ok": True,
        "status": "ok",
        "report_name": "TENNIS_LOADER_READY_BACKFILL_REPORT",
        "schema_version": "tennis_loader_ready_backfill_v1",
        ...
        "records_added_by_tennis": total_records,
        "backfill_records_written_total": total_records,
        "fields_closed_this_pass": total_records,
        "fields_partially_closed_this_pass": 0,
        "fields_reclassified_this_pass": sum(1 for row in rows if str(row["hard_block_reason"]).startswith("policy_final_state_")),
...
def write_tennis_loader_ready_backfill_report(report: dict[str, Any], *, output_dir: str | Path | None = None) -> dict[str, str]:
    root = Path(output_dir or REPORT_ROOT)
    json_path = root / "TENNIS_LOADER_READY_BACKFILL_REPORT.json"
    md_path = root / "TENNIS_LOADER_READY_BACKFILL_REPORT.md"
    write_json(json_path, report)
    lines = [
        "# Tennis Loader Ready Backfill Report",
        "",
        ...
        f"4. records_added_by_tennis: {report.get('records_added_by_tennis')}",
