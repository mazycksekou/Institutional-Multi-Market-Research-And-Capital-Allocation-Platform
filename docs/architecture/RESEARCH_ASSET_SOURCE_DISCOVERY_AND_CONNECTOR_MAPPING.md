...
| `dataset.nfl.injury_snapshots` | `sports:nfl` | Sports | Injury and availability context. | Optional | Yes | `official_team_reports`, `manual_export`, `official_team_press_releases`, `official_nfl_staff_or_news_pages` | None yet | `manual_export`, `open_web_terms_review_lanes` | Partial | Possible | Medium | Report-driven | Medium | Timing and provenance are the main leakage risks. | Terms and provenance review required. | Medium to High only if report timestamps are preserved | `src.connectors.web_scraping` or `manual_import` | `src.data` / `src.market_intelligence` | Needs Provider | Enrichment |
...
Phase 4.7B acquires the minimum certified historical dataset using the discovered source map.
