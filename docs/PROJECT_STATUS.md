...
- current phase objective: Implement Confirmed Covariance And Time-Dependent Risk Gaps is active; static covariance/correlation, covariance/correlation matrices, rolling covariance/correlation, EWMA covariance/correlation, and point-in-time covariance/correlation reconstruction are complete. The current checkpoint is Portfolio Exposure and Incremental Risk. Extend only the existing canonical math, risk, portfolio, and backtesting owners needed to answer how much risk a proposed position adds to existing portfolio exposure. Do not broaden into another market, universal capital allocation, Live Model Testing, or execution in this checkpoint.
...
- latest validation status:
  ...
  - Dynamic and Point-in-Time Covariance is complete and validated: canonical rolling covariance/correlation, EWMA covariance/correlation, and point-in-time covariance/correlation reconstruction are implemented with deterministic no-look-ahead regressions, targeted covariance tests, smoke, full local quality gates, remote Repository Validation, and merged-main verification.
...
`Portfolio Exposure and Incremental Risk`
