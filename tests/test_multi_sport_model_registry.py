...
class TestMultiSportModelRegistry(unittest.TestCase):
    ...
    def test_only_activated_nba_nfl_mlb_soccer_nhl_and_tennis_allow_confirmed_bets(self):
        sports = registry.get_sports_model_registry_response()["sports"]
        enabled = [sport["sport_key"] for sport in sports if sport["confirmed_bets_allowed"]]
        self.assertEqual(enabled, ["baseball_mlb", "basketball_nba", "americanfootball_nfl", "soccer", "icehockey_nhl", "tennis"])
        ...
        self.assertTrue(all(
            ...
            for sport in sports
            if sport["sport_key"] not in {"baseball_mlb", "basketball_nba", "americanfootball_nfl", "soccer", "icehockey_nhl", "tennis"}
        ))
    ...
    def test_endpoint_model_response_shape_validates(self):
        ...
        self.assertEqual(response["summary"]["confirmed_bet_enabled_sports"], 6)
        self.assertEqual(response["summary"]["market_derived_only_sports"], 0)
        self.assertEqual(response["summary"]["not_built_sports"], 9)
