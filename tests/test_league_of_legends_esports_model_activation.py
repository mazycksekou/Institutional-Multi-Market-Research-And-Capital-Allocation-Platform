...
def lol_alias_inputs(**extra):
    data = deepcopy(registry.get_sport_model_config("league_of_legends")["screenshot_alias_test_payload"]["input_stats"])
    data.update(extra)
    return data


def lol_inputs(**extra):
    payload = registry.get_sport_model_config("league_of_legends")["screenshot_alias_test_payload"]
...
def payload(**extra):
    data = {
        ...
        "input_stats": lol_inputs(),
    }
    data.update(extra)
    return data


class TestLeagueOfLegendsEsportsModelActivation(unittest.TestCase):
    ...
    def _screenshot(self, **extra):
        data = {
            ...
            "screenshot_text": "T1 match winner +100 vs Gen.G",
            "visible_markets": ["match_winner"],
            "input_stats": lol_alias_inputs(),
        }
        data.update(extra)
        return asyncio.run(action_analyze_ticket_screenshot(ScreenshotAnalysisRequest(**data)))

    def assert_active(self, response):
        self.assertEqual(response["model_name"], "league_of_legends_draft_objective_gold_monte_carlo_model")
        self.assertEqual(response["model_status"], "active")
        self.assertEqual(response["league_calibration_applied"], "league_of_legends")
        self.assertIsNotNone(response["final_probability"])

    def test_registry_entry_exists(self):
        config = registry.get_sport_model_config("league_of_legends")
        self.assertTrue(config["confirmed_bets_allowed"])
        self.assertEqual(config["model_used"], "league_of_legends_draft_objective_gold_monte_carlo_model")
        self.assertEqual(config["input_normalizer"], "league_of_legends_input_normalizer")
        self.assertTrue(config["screenshot_alias_test_payload"])

    def test_aliases_route_to_league_model(self):
        for alias in ("league_of_legends", "lol", "league", "riot_lol", "esports_lol", "lcs", "lec", "lck", "lpl", "worlds", "msi"):
            self.assertEqual(registry.normalize_sport_key(alias), "league_of_legends")
            self.assertEqual(registry.get_sport_model_config(alias)["model_used"], "league_of_legends_draft_objective_gold_monte_carlo_model")
    ...
    def test_low_confidence_creates_no_bet(self):
        response = self._sport(
            market="player_kills",
            selection="over",
            input_stats=lol_inputs(match_format="bo1", lan_event=False, online_event=True, substitute_risk=0.20, roster_stability=0.65, book_count=1),
        )
    ...
    def test_map_handicap_market_works(self): self.assert_active(self._sport(market="map_handicap", input_stats=lol_inputs(line=-1.5)))
    def test_total_maps_market_works(self): self.assert_active(self._sport(market="total_maps", selection="over", input_stats=lol_inputs(total_line=2.5)))
    def test_total_kills_market_works(self): self.assert_active(self._sport(market="total_kills", selection="over", input_stats=lol_inputs(total_line=29.5)))
    def test_team_total_kills_market_works(self): self.assert_active(self._sport(market="team_total_kills", selection="over", input_stats=lol_inputs(total_line=15.5)))
    def test_kill_handicap_market_works(self): self.assert_active(self._sport(market="kill_handicap", input_stats=lol_inputs(line=-3.5)))
    ...
    def test_first_dragon_market_works(self): self.assert_active(self._sport(market="first_dragon"))
    def test_first_baron_market_works(self): self.assert_active(self._sport(market="first_baron"))
    ...
    def test_player_kill_participation_prop_works(self): self.assert_active(self._sport(market="player_kill_participation", selection="over"))

    def test_bo1_calibration_works(self):
        self.assertEqual(self._sport(input_stats=lol_inputs(match_format="bo1", best_of_maps=1))["match_format_calibration_applied"], "bo1")
    ...
    def test_bo5_calibration_works(self):
        self.assertEqual(self._sport(input_stats=lol_inputs(match_format="bo5", best_of_maps=5))["match_format_calibration_applied"], "bo5")

    def test_region_calibration_works(self):
        self.assertEqual(self._sport()["region_calibration_applied"], "lck")

    def test_patch_calibration_works(self):
        self.assertEqual(self._sport(input_stats=lol_inputs(patch_version="14.11"))["patch_calibration_applied"], "14.11")
    ...
    def test_full_board_output_includes_league_fields(self):
        ...
        self.assertIn("league_of_legends_input_contract", response)
        self.assertIsNotNone(response["lol_match_probability"])
