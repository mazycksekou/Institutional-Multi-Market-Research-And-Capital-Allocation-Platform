...
class TestResponseCompactor(unittest.TestCase):
    def test_default_compact(self):
        ...
        self.assertFalse(c["provider_write"])
        self.assertFalse(c["execution_allowed"])
        self.assertFalse(c["live_execution_enabled"])
        self.assertFalse(c["auto_execution"])
        self.assertEqual(c["actual_orders_submitted"], 0)
        self.assertEqual(c["actual_bets_submitted"], 0)
        self.assertEqual(c["actual_trades_submitted"], 0)
        self.assertFalse(c["raw_payload_included"])
