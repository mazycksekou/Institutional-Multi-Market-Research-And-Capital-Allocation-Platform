...
from src.automation_scheduler_legacy.deepseek_profit_lab import run_candidate_review, run_daily_report, run_red_team_review
...
class TestDeepSeekProfitLab(unittest.TestCase):
    ...
    def test_request_uses_compact_redacted_input_only(self):
        ...
        with patch.dict(
            ...
            with patch('src.automation_scheduler_legacy.deepseek_profit_lab.httpx.Client', return_value=_FakeClient(response=fake_response)):
    ...
    def test_timeout_returns_safe_provider_timeout(self):
        with patch.dict(os.environ, {"DEEPSEEK_ENABLED": "true", "DEEPSEEK_API_KEY": "sk-test-secret-1234567890"}, clear=True):
            with patch('src.automation_scheduler_legacy.deepseek_profit_lab.httpx.Client', return_value=_FakeClient(exc=httpx.TimeoutException("timeout"))):
                result = run_candidate_review(candidate=_candidate())
