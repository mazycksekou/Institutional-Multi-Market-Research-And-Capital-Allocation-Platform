from __future__ import annotations

import ast
import importlib
import os
import sys
from pathlib import Path

import pytest


FORBIDDEN_IMPORTS = {"requests", "httpx", "websocket", "yfinance", "selenium", "playwright", "openai", "anthropic", "alpaca", "robinhood", "ib_insync", "ccxt"}


def _clear_brokerage_modules() -> None:
    for name in list(sys.modules):
        if name == "src.brokerage" or name.startswith("src.brokerage."):
            sys.modules.pop(name, None)


def _fresh_import(name: str, monkeypatch: pytest.MonkeyPatch):
    _clear_brokerage_modules()

    def _forbidden_getenv(*args, **kwargs):
        raise AssertionError("os.getenv must not be called at import time")

    monkeypatch.setattr(os, "getenv", _forbidden_getenv)
    return importlib.import_module(name)


def _assert_no_forbidden_imports(module) -> None:
    source = Path(module.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source)
    imports: set[str] = set()
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                imports.add(alias.name.split(".")[0])
        elif isinstance(node, ast.ImportFrom) and node.module:
            imports.add(node.module.split(".")[0])
    assert imports.isdisjoint(FORBIDDEN_IMPORTS), imports & FORBIDDEN_IMPORTS


def test_credential_readiness(monkeypatch: pytest.MonkeyPatch) -> None:
    module = _fresh_import("src.brokerage.credential_readiness", monkeypatch)
    _assert_no_forbidden_imports(module)

    default_state = module.build_disabled_credential_readiness(broker_name="sandbox-broker")
    assert default_state.ready is False
    assert default_state.live_trading_allowed is False
    assert default_state.credentials_required is True

    readiness = module.CredentialReadinessState(
        broker_name="sandbox-broker",
        requirements=(
            module.CredentialRequirement(name="api_key", required=True, satisfied=True),
            module.CredentialRequirement(name="api_secret", required=True, satisfied=True),
        ),
        status="ready_local_only",
        ready=True,
        credentials_required=True,
        live_trading_allowed=False,
    )
    result = module.evaluate_credential_readiness(readiness)
    assert result.ready is True
    assert result.live_trading_allowed is False
    assert result.status == "ready_local_only"

