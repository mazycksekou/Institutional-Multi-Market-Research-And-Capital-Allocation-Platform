...
def test_nfl_weather_research_asset_population_docs_exist_and_cover_required_topics() -> None:
    architecture_doc = DOCS / "architecture" / "NFL_WEATHER_RESEARCH_ASSET.md"
    report = DOCS / "reports" / "PHASE4_9F_NFL_WEATHER_RESEARCH_ASSET_POPULATION.md"
    ...
    assert "NFL Weather Research Asset" in architecture_text
    assert "dataset.nfl.weather_snapshots" in architecture_text
    assert "schedule, results, and odds backbone" in architecture_text.lower()
    assert "field-level provenance" in architecture_text.lower()
    assert "FEATURE_READY" in architecture_text
    assert "pregame forecast" in architecture_text.lower()

    assert "Phase 4.9F - NFL Weather Research Asset Population" in report_text
    ...
    assert "Readiness For Phase 4.9G" in report_text
    assert "Senior Systems Engineer Review" in report_text
    assert "Worldview Intelligence Review" in report_text

    assert "Phase 4.9F - NFL Weather Research Asset Population (complete)" in project_status_text
    assert "Phase 4.9G - NFL Injuries Research Asset Population" in next_action_text
    assert "docs/architecture/NFL_WEATHER_RESEARCH_ASSET.md" in project_status_text
    assert "docs/reports/PHASE4_9F_NFL_WEATHER_RESEARCH_ASSET_POPULATION.md" in project_status_text
    assert "docs/architecture/NFL_WEATHER_RESEARCH_ASSET.md" in master_index_text
    assert "docs/reports/PHASE4_9F_NFL_WEATHER_RESEARCH_ASSET_POPULATION.md" in retention_index_text
    assert "injuries" in next_action_text.lower()


def test_nfl_weather_research_asset_population_docs_do_not_depend_on_runtime_code() -> None:
    architecture_doc = DOCS / "architecture" / "NFL_WEATHER_RESEARCH_ASSET.md"
    report = DOCS / "reports" / "PHASE4_9F_NFL_WEATHER_RESEARCH_ASSET_POPULATION.md"
