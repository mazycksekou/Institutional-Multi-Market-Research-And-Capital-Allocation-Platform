...
MODEL_NAME = "pickleball_dink_kitchen_serve_return_monte_carlo_model"
ALIASES = ("pickleball", "pro_pickleball", "ppa", "mlf", "major_league_pickleball", "app_tour", "pickleball_singles", "pickleball_doubles")


def pickleball_alias_inputs(**extra):
    data = deepcopy(registry.get_sport_model_config("pickleball")["screenshot_alias_test_payload"]["input_stats"])
    data.update(extra)
    return data


def pickleball_inputs(**extra):
    ticket = registry.get_sport_model_config("pickleball")["screenshot_alias_test_payload"]
...
def payload(**extra):
    data = {
        "sport": "pickleball", "league": "PPA Tour", "event_id": "Ben Johns vs Federico Staksrud",
        "event": "Ben Johns vs Federico Staksrud", "market": "match_winner", "selection": "Ben Johns",
        "odds_american": 100, "book": "Manual", "bankroll": 1000, "unit_size": 25,
        "risk_profile": "moderate", "source_type": "unit_test",
        "screenshot_text": "Ben Johns match winner +100 vs Federico Staksrud", "visible_markets": ["match_winner"],
        "input_stats": pickleball_inputs(),
    }
    data.update(extra)
    return data


class TestPickleballModelActivation(unittest.TestCase):
    ...
    def _screenshot(self, **extra):
        data = {
            "source_type": "chatgpt_parsed", "sport": "ppa", "league": "PPA Tour",
            "event": "Ben Johns vs Federico Staksrud", "market": "match_winner",
            "selection": "Ben Johns", "odds_american": 100, "book": "Manual",
            "bankroll": 1000, "unit_size": 25, "risk_profile": "moderate",
            "screenshot_text": "Ben Johns match winner +100 vs Federico Staksrud",
            "visible_markets": ["match_winner"], "input_stats": pickleball_alias_inputs(),
        }
        data.update(extra)
    ...
    def assert_active(self, response):
        ...
        self.assertEqual(response["league_calibration_applied"], "pickleball")
        self.assertIsNotNone(response["final_probability"])

    def test_registry_entry_exists(self):
        config = registry.get_sport_model_config("pickleball")
        self.assertTrue(config["confirmed_bets_allowed"])
        self.assertEqual(config["model_used"], MODEL_NAME)
        self.assertEqual(config["input_normalizer"], "pickleball_input_normalizer")
        self.assertTrue(config["screenshot_alias_test_payload"])

    def test_aliases_route_to_pickleball_model(self):
        for alias in ALIASES:
            self.assertEqual(registry.normalize_sport_key(alias), "pickleball")
    ...
    def test_malformed_text_inputs_do_not_activate(self):
        malformed = pickleball_alias_inputs(serve_rating="bad", return_rating="bad", rally_rating="text", ball_speed="bad")
    ...
    def test_low_confidence_creates_no_bet(self):
        response = self._sport(input_stats=pickleball_inputs(book_count=1, fatigue_rating=0.80, injury_risk=0.38))
    ...
    def test_public_sharp_market_movement_are_enrichment_only(self):
        ...
        enriched = self._sport(input_stats=pickleball_inputs(public_betting_percent=99, sharp_money_percent=1, market_movement=9))["final_probability"]
    ...
    def test_game_handicap_market_works(self): self.assert_active(self._sport(market="game_handicap", input_stats=pickleball_inputs(line=-1.5)))
    def test_point_handicap_market_works(self): self.assert_active(self._sport(market="point_handicap", input_stats=pickleball_inputs(line=-4.5)))
    def test_total_games_market_works(self): self.assert_active(self._sport(market="total_games", selection="over", input_stats=pickleball_inputs(line=2.5)))
    def test_total_points_market_works(self): self.assert_active(self._sport(market="total_points", selection="over", input_stats=pickleball_inputs(line=78.5)))
    ...
    def test_alt_game_handicap_market_works(self): self.assert_active(self._sport(market="alt_game_handicap", input_stats=pickleball_inputs(line=-2.5)))
    def test_alt_total_games_market_works(self): self.assert_active(self._sport(market="alt_total_games", selection="over", input_stats=pickleball_inputs(line=2.5)))
    def test_alt_total_points_market_works(self): self.assert_active(self._sport(market="alt_total_points", selection="over", input_stats=pickleball_inputs(line=82.5)))

    def test_player_total_points_prop_works(self): self.assert_active(self._sport(market="player_total_points", selection="Ben Johns over", input_stats=pickleball_inputs(line=42.5)))
    def test_team_total_points_prop_works(self): self.assert_active(self._sport(market="team_total_points", selection="Ben Johns over", input_stats=pickleball_inputs(line=42.5)))
    def test_player_aces_prop_works(self): self.assert_active(self._sport(market="player_aces", selection="Ben Johns over", input_stats=pickleball_inputs(line=0.5)))
    def test_player_service_points_won_prop_works(self): self.assert_active(self._sport(market="player_service_points_won", selection="Ben Johns over", input_stats=pickleball_inputs(line=25.5)))
    def test_player_return_points_won_prop_works(self): self.assert_active(self._sport(market="player_return_points_won", selection="Ben Johns over", input_stats=pickleball_inputs(line=17.5)))
    ...
    def test_calibration_outputs_exist(self):
        ...
        self.assertEqual(response["tournament_calibration_applied"], "ppa")
        self.assertTrue(response["serve_return_calibration_applied"])
        self.assertTrue(response["kitchen_calibration_applied"])
        self.assertTrue(response["deciding_game_calibration_applied"])

    def test_doubles_and_unknown_calibrations(self):
        self.assertEqual(self._sport(input_stats=pickleball_inputs(singles_doubles="doubles"))["discipline_calibration_applied"], "doubles")
        self.assertEqual(self._sport(input_stats=pickleball_inputs(best_of_games=5, match_format="custom"))["format_calibration_applied"], "best_of_5")
        self.assertEqual(self._sport(input_stats=pickleball_inputs(best_of_games=7, match_format="custom"))["format_calibration_applied"], "unknown")
        self.assertEqual(self._sport(input_stats=pickleball_inputs(tournament="Friendly Series", competition="Friendly"))["tournament_calibration_applied"], "unknown")

    def test_serve_return_and_rally_edges_affect_probability(self):
        base = self._sport()["final_probability"]
        weaker = self._sport(input_stats=pickleball_inputs(player_serve_rating=78, player_return_rating=76, player_rally_rating=77, player_kitchen_rating=76))["final_probability"]
    ...
    def test_full_board_output_includes_pickleball_fields(self):
        response = self._sport()
        ...
        self.assertIn("pickleball_input_contract", response)
        self.assertIsNotNone(response["pickleball_projected_total_points"])

    def test_logbook_fields_exist(self):
        row = self._sport()["logbook_ready_rows"][0]
        for field in ("confidence", "model_status", "decision", "stake", "suggested_stake", "league_calibration_applied", "format_calibration_applied", "discipline_calibration_applied", "tournament_calibration_applied", "serve_return_calibration_applied", "kitchen_calibration_applied", "deciding_game_calibration_applied"):
            self.assertIn(field, row)

    def test_input_contract_contains_required_groups(self):
        contract = self._sport()["pickleball_input_contract"]
