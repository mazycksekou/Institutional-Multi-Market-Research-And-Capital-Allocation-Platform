...
from automation_scheduler.nhl_free_open_exhaustion import (
    build_nhl_final_oxylabs_free_open_exhaustion_report,
    write_nhl_final_oxylabs_free_open_exhaustion_report,
)
...
SAMPLE = {"sample_results": [{"sample_type": "one_game", "validation_status": "sample_verified"}]}
AUDIT = {
...
BACKFILL = {"loader_ready_lanes_before": 1, "loader_ready_lanes_backfilled": 1, "loader_ready_lanes_hard_blocked": 0, "records_added_by_nhl": 10, "fields_closed_this_pass": 10, "fields_partially_closed_this_pass": 0}
RECLASS = {"reclassification_row_count": 2}
SCHEMA = {"new_fields_created_count": 3, "new_tables_created_count": 2}
PAID = {"requirement_rows": [{"lane_name": "goalie_gsaax_dataset"}]}
READINESS = {"models": [{"recommendation": "ready_but_paid_data_would_improve"}]}
CERT = {"free_open_exhaustion_verified": True}


class TestNhlFinalOxylabsFreeOpenExhaustionReport(unittest.TestCase):
    def test_final_report_declares_exhausted(self):
        fake_lanes = [{"lane_name": "schedule_results"}] * 4
        with patch("automation_scheduler.nhl_free_open_exhaustion.nhl_lane_catalog", return_value=fake_lanes), patch(
            "automation_scheduler.nhl_free_open_exhaustion.build_nhl_source_exhaustion_query_plan",
            ...
            report = build_nhl_final_oxylabs_free_open_exhaustion_report(
        ...
        self.assertEqual(report["new_overall_verdict"], "NHL_FINAL_FREE_OPEN_EXHAUSTED")
    ...
    def test_writer_creates_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_nhl_final_oxylabs_free_open_exhaustion_report(report, output_dir=Path(tmp) / "reports")
