from __future__ import annotations

import ast
import importlib
import os
import sys
from pathlib import Path

import pytest


FORBIDDEN_IMPORTS = {"requests", "httpx", "websocket", "yfinance", "selenium", "playwright", "openai", "anthropic", "alpaca", "robinhood", "ib_insync", "ccxt"}


def _clear_brokerage_modules() -> None:
    for name in list(sys.modules):
        if name == "src.brokerage" or name.startswith("src.brokerage."):
            sys.modules.pop(name, None)


def _fresh_import(name: str, monkeypatch: pytest.MonkeyPatch):
    _clear_brokerage_modules()

    def _forbidden_getenv(*args, **kwargs):
        raise AssertionError("os.getenv must not be called at import time")

    monkeypatch.setattr(os, "getenv", _forbidden_getenv)
    return importlib.import_module(name)


def _assert_no_forbidden_imports(module) -> None:
    source = Path(module.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source)
    imports: set[str] = set()
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                imports.add(alias.name.split(".")[0])
        elif isinstance(node, ast.ImportFrom) and node.module:
            imports.add(node.module.split(".")[0])
    assert imports.isdisjoint(FORBIDDEN_IMPORTS), imports & FORBIDDEN_IMPORTS


def test_approval_evidence(monkeypatch: pytest.MonkeyPatch) -> None:
    module = _fresh_import("src.brokerage.approval_evidence", monkeypatch)
    _assert_no_forbidden_imports(module)

    default_evidence = module.build_default_approval_evidence()
    assert default_evidence.approved is False
    assert default_evidence.status == "not_approved"

    result = module.validate_approval_evidence(default_evidence)
    assert result.valid is False
    assert result.approved is False
    assert result.status == "not_approved"

    approved_evidence = module.ApprovalEvidence(
        evidence_id="approval_evidence_explicit",
        source=module.ApprovalSource.OWNER,
        requirements=tuple(
            module.ApprovalRequirement(
                name=req.name,
                required=req.required,
                satisfied=True,
                description=req.description,
            )
            for req in module.build_default_approval_evidence().requirements
        ),
        approved=True,
        status="approved_local_only",
        approval_scope="sandbox_activation",
    )
    approved = module.validate_approval_evidence(approved_evidence)
    assert approved.valid is True
    assert approved.approved is True
    assert approved.status == "approved_local_only"

