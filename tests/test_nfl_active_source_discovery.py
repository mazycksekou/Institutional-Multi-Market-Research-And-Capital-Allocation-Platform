...
class TestNflActiveSourceDiscovery(unittest.TestCase):
    def test_log_contains_accepted_and_rejected_sources(self):
        report = build_active_source_discovery_log(sport="nfl", allow_oxylabs=True, allow_paid_retrieval=True)
    ...
    def test_log_writes_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            report = build_active_source_discovery_log(sport="nfl", allow_oxylabs=True, allow_paid_retrieval=True)
            paths = write_active_source_discovery_log(report, output_dir=Path(tmp) / "reports", sport="nfl")
        self.assertTrue(paths["latest_json_path"].endswith("reports/NFL_ACTIVE_SOURCE_DISCOVERY_LOG.json"))
