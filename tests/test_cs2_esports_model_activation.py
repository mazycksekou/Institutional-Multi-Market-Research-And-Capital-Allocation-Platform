...
def cs2_alias_inputs(**extra):
    data = deepcopy(registry.get_sport_model_config("cs2")["screenshot_alias_test_payload"]["input_stats"])
    data.update(extra)
    return data


def cs2_inputs(**extra):
    payload = registry.get_sport_model_config("cs2")["screenshot_alias_test_payload"]
...
def payload(**extra):
    data = {
        ...
        "event_id": "Natus Vincere vs FaZe Clan",
        "event": "Natus Vincere vs FaZe Clan",
        ...
        "input_stats": cs2_inputs(),
    }
    data.update(extra)
    return data


class TestCS2EsportsModelActivation(unittest.TestCase):
    ...
    def _screenshot(self, **extra):
        data = {
            ...
            "screenshot_text": "Natus Vincere match winner +100 vs FaZe Clan",
            "visible_markets": ["match_winner"],
            "input_stats": cs2_alias_inputs(),
        }
        data.update(extra)
        return asyncio.run(action_analyze_ticket_screenshot(ScreenshotAnalysisRequest(**data)))

    def assert_active(self, response):
        self.assertEqual(response["model_name"], "cs2_round_economy_map_pool_monte_carlo_model")
        self.assertEqual(response["model_status"], "active")
        self.assertEqual(response["league_calibration_applied"], "cs2")
        self.assertIsNotNone(response["final_probability"])

    def test_registry_entry_exists(self):
        config = registry.get_sport_model_config("cs2")
        self.assertTrue(config["confirmed_bets_allowed"])
        self.assertEqual(config["model_used"], "cs2_round_economy_map_pool_monte_carlo_model")
        self.assertEqual(config["input_normalizer"], "cs2_input_normalizer")
        self.assertTrue(config["screenshot_alias_test_payload"])

    def test_aliases_route_to_cs2_model(self):
        for alias in ("cs2", "counter_strike_2", "counterstrike2", "counter_strike", "csgo", "counterstrike", "esports_cs2"):
            self.assertEqual(registry.normalize_sport_key(alias), "cs2")
            self.assertEqual(registry.get_sport_model_config(alias)["model_used"], "cs2_round_economy_map_pool_monte_carlo_model")
    ...
    def test_malformed_text_inputs_do_not_activate(self):
        malformed = cs2_alias_inputs()
        for key in ("team_elo_rating", "team_win_pct", "team_round_win_pct", "team_ct_pct", "team_adr_value"):
    ...
    def test_low_confidence_creates_no_bet(self):
        response = self._sport(
            market="player_kills",
            selection="over",
            input_stats=cs2_inputs(match_format="bo1", lan_event=False, online_event=True, substitute_risk=0.20, roster_stability=0.65, book_count=1),
        )
    ...
    def test_map_handicap_market_works(self): self.assert_active(self._sport(market="map_handicap", input_stats=cs2_inputs(line=-1.5)))
    def test_total_maps_market_works(self): self.assert_active(self._sport(market="total_maps", selection="over", input_stats=cs2_inputs(total_line=2.5)))
    def test_total_rounds_market_works(self): self.assert_active(self._sport(market="total_rounds", selection="over", input_stats=cs2_inputs(total_line=51.5)))
    def test_team_total_rounds_market_works(self): self.assert_active(self._sport(market="team_total_rounds", selection="over", input_stats=cs2_inputs(total_line=26.5)))
    ...
    def test_player_headshots_prop_works(self): self.assert_active(self._sport(market="player_headshots", selection="over"))
    ...
    def test_player_opening_kills_prop_works(self): self.assert_active(self._sport(market="player_opening_kills", selection="over"))

    def test_bo1_calibration_works(self):
        self.assertEqual(self._sport(input_stats=cs2_inputs(match_format="bo1", best_of_maps=1))["match_format_calibration_applied"], "bo1")
    ...
    def test_bo5_calibration_works(self):
        self.assertEqual(self._sport(input_stats=cs2_inputs(match_format="bo5", best_of_maps=5))["match_format_calibration_applied"], "bo5")
    ...
    def test_online_calibration_works(self):
        response = self._sport(input_stats=cs2_inputs(lan_event=False, online_event=True))
        self.assertEqual(response["event_environment_calibration_applied"], "online")

    def test_map_calibration_works(self):
        self.assertEqual(self._sport(input_stats=cs2_inputs(map_name="Inferno"))["map_calibration_applied"], "Inferno")
    ...
    def test_full_board_output_includes_cs2_fields(self):
        response = self._sport()
        ...
        self.assertIn("cs2_input_contract", response)
        self.assertIsNotNone(response["cs2_match_probability"])

    def test_logbook_fields_exist(self):
        row = self._sport()["logbook_ready_rows"][0]
        for field in ("confidence", "model_status", "decision", "stake", "suggested_stake", "league_calibration_applied", "match_format_calibration_applied", "event_environment_calibration_applied", "map_calibration_applied"):
