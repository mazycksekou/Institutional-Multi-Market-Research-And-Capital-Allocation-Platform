...
from automation_scheduler.streamlit_dashboard_data import build_paper_only_fixture_readiness_payload
from quant_engine import evaluate_paper_only_fixture_rows


REPO_ROOT = Path(__file__).resolve().parents[1]
REPORT_PATH = REPO_ROOT / "PHASE10K8F_PAPER_ONLY_FIXTURE_EVALUATION_HELPER.md"
...
def test_paper_only_fixture_evaluation_helper_round_trip():
    valid_row = {
    ...
    assert evaluation_result["rows_tested"] == 1
    assert evaluation_result["rows_valid"] == 1
    assert evaluation_result["rows_invalid"] == 0
    assert len(evaluation_result["evaluations"]) == 1
    evaluation = evaluation_result["evaluations"][0]
    assert evaluation["paper_edge"] == valid_row["model_probability"] - valid_row["implied_probability"]
    assert evaluation["paper_ev"] == valid_row["expected_value"]
    assert evaluation["paper_stake_units"] == valid_row["stake_units"]
    assert evaluation["paper_result"] == "pending"
    assert evaluation_result["prediction_testing_started"] is False
    assert evaluation_result["live_connectors_enabled"] is False
    assert evaluation_result["api_calls_enabled"] is False
    assert evaluation_result["database_writes_enabled"] is False

    winning_row = dict(valid_row)
    winning_row["outcome_known"] = True
    winning_row["result_label"] = "won by margin"
    winning_result = evaluate_paper_only_fixture_rows([winning_row])
    assert winning_result["evaluations"][0]["paper_result"] == "paper_win"

    invalid_row = dict(valid_row)
    invalid_row.pop("selection")
    invalid_result = evaluate_paper_only_fixture_rows([invalid_row])
    assert invalid_result["rows_invalid"] == 1
    assert invalid_result["missing_field_reasons"]

    readiness_payload = build_paper_only_fixture_readiness_payload(evaluation_result)
    assert readiness_payload["rows_tested"] == evaluation_result["rows_tested"]
    assert readiness_payload["rows_valid"] == evaluation_result["rows_valid"]
    assert readiness_payload["rows_invalid"] == evaluation_result["rows_invalid"]
    assert readiness_payload["missing_field_reasons"] == evaluation_result["missing_field_reasons"]
    assert readiness_payload["warning_reasons"] == evaluation_result["warning_reasons"]
